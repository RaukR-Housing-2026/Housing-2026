...
animate_form <- function(form, gif_name) {
  ...
  bb <- sf::st_bbox(municipality)
  year_label <- tibble(
    year = sort(unique(kommun_map$year)),
  ...
  anim <- kommun_map |>
    ...
    scale_fill_gradientn(colours = rev(paletteer::paletteer_c("grDevices::Reds 3", 30)),
                         ...
                         labels = scales::label_number(),
                         na.value = "grey85") +
    labs(title = paste0("Median price, ", form, " (Hemnet)"),
         fill = "SEK") +
    theme_swemap() +
  ...
  animate(anim, fps = 12, nframes = 300, width = 1050, height = 900, res = 100,
